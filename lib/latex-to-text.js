...
function formatSubscript(content) {
  const chars = content.split('').map(c => SUBSCRIPT_MAP[c]);
  if (chars.every(c => c !== undefined)) return chars.join('');
  return '\u208D' + content + '\u208E';
}
...
export function latexToText(latex) {
  ...
  result = result.replace(/\^\{([^}]*)\}/g, (_, c) => {
    if (/^-?[\d]+$/.test(c)) return c.split('').map(ch => SUPERSCRIPT_MAP[ch] || ch).join('');
    return c.length > 1 ? '^(' + c + ')' : (SUPERSCRIPT_MAP[c] || '^' + c);
  });
  result = result.replace(/\^([0-9])/g, (_, c) => SUPERSCRIPT_MAP[c] || '^' + c);
